...
# Verifies the generic ad layer + game after the Unity Ads Legacy -> LevelPlay
# swap, via the EDITOR (Mock provider) code path. LevelPlayAdProvider is NOT
# #if-gated, so it always compiles and needs the Unity.LevelPlay assembly.
$refs = @()
...
$rsp = "$env:TEMP\wd_compile_levelplay.rsp"
$args = @("-nologo", "-target:library", "-nowarn:0168,0219,0414,0649", "-define:UNITY_EDITOR;UNITY_2021_1_OR_NEWER;UNITY_6000", "-out:`"$env:TEMP\wd_check_levelplay.dll`"") + $refs + $sources
