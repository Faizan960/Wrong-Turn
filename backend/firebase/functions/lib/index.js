...
exports.getRankCard = exports.submitScore = exports.updateRegion = exports.updateDisplayName = exports.ensureProfile = void 0;
// ============================================================================
...
function requireUid(auth) {
    if (!auth?.uid)
        throw new https_1.HttpsError("unauthenticated", "NOT_AUTHENTICATED");
    return auth.uid;
}
function sanitizeName(raw) {
    ...
    if (PROFANITY.test(name))
        throw new https_1.HttpsError("invalid-argument", "NAME_REJECTED");
    return name;
}
...
// ---------------------------------------------------------------------------
exports.ensureProfile = (0, https_1.onCall)(async (req) => {
    ...
    let name;
    try {
        name = sanitizeName(req.data?.defaultName);
    }
    catch {
        name = randomAlias();
    }
...
// ---------------------------------------------------------------------------
exports.updateDisplayName = (0, https_1.onCall)(async (req) => {
    const uid = requireUid(req.auth);
    ...
    if ((await lb.get()).exists)
        batch.update(lb, { displayName: name });
    await batch.commit();
...
// ---------------------------------------------------------------------------
exports.updateRegion = (0, https_1.onCall)(async (req) => {
    const uid = requireUid(req.auth);
    ...
    const snap = await ref.get();
    if (!snap.exists)
        throw new https_1.HttpsError("failed-precondition", "NO_PROFILE");
    const locked = snap.get("regionChangedAt");
    const cooldownMs = 30 * 24 * 60 * 60 * 1000;
    if (locked && Date.now() - locked.toMillis() < cooldownMs) {
        const nextMs = locked.toMillis() + cooldownMs;
        throw new https_1.HttpsError("failed-precondition", "REGION_LOCKED", { nextChangeAt: nextMs });
    }
    const patch = {
        countryCode, countryDisplay, cityId, cityDisplay,
        regionChangedAt: firestore_1.FieldValue.serverTimestamp(),
    };
    const batch = db.batch();
...
// ---------------------------------------------------------------------------
exports.submitScore = (0, https_1.onCall)(async (req) => {
    const uid = requireUid(req.auth);
    const d = req.data ?? {};
    ...
    // ---- Plausibility gate ----------------------------------------------------
    if (easyMode || daily)
        throw new https_1.HttpsError("failed-precondition", "NON_COMPETITIVE_MODE");
    for (const v of [score, maxCombo, correct, wrong, duration]) {
        if (!Number.isFinite(v) || v < 0)
            throw new https_1.HttpsError("invalid-argument", "NEGATIVE_OR_NAN");
    }
    if (score > ABSOLUTE_SCORE_CEILING)
        throw new https_1.HttpsError("invalid-argument", "SCORE_CEILING");
    if (maxCombo > correct)
        throw new https_1.HttpsError("invalid-argument", "COMBO_EXCEEDS_CORRECT");
    if (duration > 0 && correct > duration * MAX_ANSWERS_PER_SEC)
        throw new https_1.HttpsError("invalid-argument", "IMPOSSIBLE_ANSWER_RATE");
    if (score > (correct + 1) * MAX_POINTS_PER_CORRECT)
        throw new https_1.HttpsError("invalid-argument", "SCORE_INCONSISTENT_WITH_ANSWERS");
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const patch = { ...base, score: newScore, comboScore: newCombo };
        if (scoreImproved || !lbSnap.exists)
            patch.achievedAt = now;
        if (comboImproved || !lbSnap.exists)
            patch.comboAchievedAt = now;
    ...
    const card = await computeRankCard(uid, "high_score", rulesetVersion, result.newScore, result.achievedAt);
    return { ok: true, card };
});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
exports.getRankCard = (0, https_1.onCall)(async (req) => {
    const uid = requireUid(req.auth);
    ...
    const myAchievedAt = lb.get("achievedAt");
...
// ---------------------------------------------------------------------------
async function computeRankCard(uid, board, ruleset, myScore, myAchievedAt) {
    if (!ALLOWED_BOARDS.has(board))
        throw new https_1.HttpsError("invalid-argument", "BAD_BOARD");
