...
namespace UnityEngine.Advertisements.Editor
{
    static class MobileDependencyResolverInstallerUnity
    {
        ...
        {
            //Do not install when running in CI
            if (Environment.GetEnvironmentVariable("UNITY_THISISABUILDMACHINE") != null) {
                return;
            }

            //Do not attempt if already installed
            if (IsPackageInstalled(k_UnityMediationPackage) || IsPlayServicesResolverInstalled()) {
                return;
            }

            //Delete dialog preference on exit if it was saved
            EditorApplication.quitting += EditorApplicationOnQuitting;

            //If running batch mode or if the user was prompted and said yes then install
            if (Application.isBatchMode || AskUserToInstallPackage()) {
                InstallPackage();
            }
        }
